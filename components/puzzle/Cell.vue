...
const cellClass = (cell) => ({
  ...
  '!bg-gray-800 hover_!bg-gray-700': cell.x == puzzle.x || cell.y == puzzle.y || (cell.value && cell.value == puzzle.currentCell.value),
  '!bg-gray-600 hover_!bg-gray-500': cell.x == puzzle.x && cell.y == puzzle.y,
  // '!bg-blue-300': cell.value == puzzle.currentCell.value
})

const noteClass = (cell, note) => ({
  '!bg-gray-800 hover_!bg-gray-700': note == puzzle.currentCell.value && cell.notes.includes(note)
})
</script>
...
div(:class="cellClass(cell)" @click="puzzle.select(cell)").aspect-square.flex.justify-center.items-center.text-6xl.text-gray-300.bg-gray-900.hover_bg-gray-800.font-light
  ...
  div(v-else).grid.grid-cols-3.w-full
    div(v-for="x in 9" :key="x" :class="noteClass(cell, x)").aspect-square.flex.justify-center.items-center.text-sm
